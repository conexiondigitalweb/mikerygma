...
| IA | Anthropic API — `claude-sonnet-4-6` primario, fallback automático a `claude-haiku-4-5` solo si Sonnet bloquea por content filtering. `maxDuration: 300s` en `api/generate.js` (ver `vercel.json`) |
...
Cuatro pasos secuenciales, cada uno con degradación silenciosa: si un paso posterior a la generación principal falla, el usuario igual recibe el sermón completo sin ese extra — nunca se rompe la respuesta por un paso opcional.

**1. Generación principal con fallback por content filtering.** Modelo primario `claude-sonnet-4-6`. Si Anthropic bloquea la respuesta por content filtering (diagnosticado: Sonnet bloqueó Lucas 15:11-32 en 4/4 pruebas con el mismo prompt; Haiku nunca lo bloqueó en 3/3), se reintenta **una sola vez** con `claude-haiku-4-5` (`FALLBACK_MAX_TOKENS=8000` — en las pruebas, los JSON completos de Haiku para este contenido rondaron 10,200-10,550 caracteres y se truncaban con el límite de 3500 tokens de Sonnet; 8000 da ~2x margen y se mantiene bajo el techo de 8192 tokens de salida de Haiku sin headers beta). Cualquier otro tipo de error (HTTP, red) no reintenta. El modelo que ganó viaja al cliente en el header `X-Model-Used` y se guarda en `generations.model_used`.
...
2. **`max_tokens: 3500`** en generate.js (Sonnet) / **8000** en el fallback (Haiku) para evitar truncamiento del JSON
